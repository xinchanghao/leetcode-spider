 ...
 * @api public
 */
let dirExistsOrCreate = co.wrap(function*(outputDir, path){
    try{
        let stats = yield thenFs.stat(outputDir);
        if(!stats.isDirectory()){
            throw new Error('not directory');
        }
    }catch(e){
        yield thenFs.mkdir(outputDir);
    }
    try{
 ...
 * @api public
 */
exports.writeToFile = co.wrap(function*(languageCodeMap, outputDir){
    yield dirExistsOrCreate(outputDir, path.resolve(process.cwd(),outputDir,formatId(languageCodeMap._id)+'.'+languageCodeMap._title));
    try{
        let solveLang = [];
        let promises = Object.keys(languageCodeMap).filter((prop)=>{
            ...
            solveLang.push(lang);
            return write(path.resolve(process.cwd(),outputDir,formatId(languageCodeMap._id)+'.'+languageCodeMap._title,languageCodeMap._title+'.'+langExtMap[lang]),languageCodeMap[lang]);
        })
        languageCodeMap._solveLang = solveLang;
        promises.push(write(path.resolve(process.cwd(),outputDir,formatId(languageCodeMap._id)+'.'+languageCodeMap._title,'question.md'),languageCodeMap['_question']))
        yield promises;
    }catch(err){
