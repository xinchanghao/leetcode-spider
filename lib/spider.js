...
exports.fetch = co.wrap(function*(conf,numObj){
    ...
    try{
        yield parallelFetch(fetchList, config.outputDir)
    }catch(e){
        throw e;
    }
})
 ...
 * @api public
 */
let parallelFetch = co.wrap(function*(acLists, outputDir){
    ...
    if(acLists && acLists.length > 0){
        ...
        let acListPromises = acLists.map(acProblem => {
            ...
            return fetchAndWrite(acProblem,languageClone,languageCodeMap, outputDir);
        })
        ...
        try{
            yield generateMDUtils.generateMarkdown(resultObj,leetcodeNumObj, outputDir);
        }catch(e){
            throw e;
        }
 ...
 * @api public
 */
let fetchAndWrite = function*(problemInfo,languageToFetch,languageCodeMap,outputDir){
    ...
    yield fileUtils.writeToFile(languageCodeMap,outputDir).catch(err => {throw err});
